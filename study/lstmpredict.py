...
with graph.as_default():
    ...
    #print(outputs.shape)
    h=outputs[:,-1,:]
    w = tf.Variable(tf.random_uniform((5,1)),dtype=tf.float32)
    y = tf.matmul(h,w)
    #print(h.shape)
    #--------------------------------------------------------------------------------------------#

    #---------------------------------define loss and optimizer----------------------------------#
    mse=tf.losses.mean_squared_error(labels=y_p,predictions=y)
    #print(loss.shape)
...
with tf.Session(graph=graph) as sess:
    ...
    for epoch in range(1,EPOCH+1):
        ...
        for j in range(TRAIN_EXAMPLES//BATCH_SIZE):
            if j == 2:
                _, train_loss = sess.run(
                    ...
                    feed_dict={
                        X_p: X_train[j * BATCH_SIZE:(j + 1) * BATCH_SIZE-1],
        ...
        plt.plot(range(1000),results[:1000,0], label='%d epoch'%epoch)
    plt.legend()
    plt.show()
