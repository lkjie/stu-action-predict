...
def build_model():
    ...
    branch_outputs = []
    out = Lambda(lambda x: x[:, :, 0])(fea_inp)
    emb = Embedding(input_dim=emb_cates[0], output_dim=100, input_length=maxlen, mask_zero=False, trainable=True,
                    name=emb_names[0])(out)
    branch_outputs.append(emb)
    time_inp = Lambda(lambda x: x[:, :, 1])(fea_inp)
    time_onehot = OneHot(input_dim=timeslot_cates, input_length=1)(time_inp)
    branch_outputs.append(time_onehot)
