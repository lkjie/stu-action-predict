...
def build_model():
    ...
    for i in range(timeseries_count):
        ...
        if timeseries[i] == 'student_id_int':
            nextlayer = Embedding(input_dim=emb_timeseries_cates[i], output_dim=100, input_length=maxlen, mask_zero=False, trainable=True, name=emb_timeseries_names[i])(out)
        else:
            nextlayer = OneHot(input_dim=emb_timeseries_cates[i], input_length=1)(out)
    ...
    for i in range(feature_count):
        ...
        if timeseries[i] == 'student_id_int':
            nextlayer = Embedding(input_dim=emb_feat_cates[i], output_dim=100, mask_zero=False, trainable=True,
