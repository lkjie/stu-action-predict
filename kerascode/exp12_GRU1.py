...
labels = ['placei', 'amount', 'timeslot_week']
...
xlist, ylist = load_data_exptl(timeseries, labels, 12)
placelist = ylist[:, 0]
amountlist = ylist[:, 1]
timeslot_weeklist = ylist[:, 2]
if stratify:
    ...
    timeslot_weeklist = timeslot_weeklist[idy]
    x_train1, x_test1, y_train1, y_test1, y_train2, y_test2, y_train3, y_test3 = train_test_split(xlist,
...
def build_model():
    ...
    model.compile(loss=[sparse_focal_loss, 'mse', 'mse'],
                  optimizer='adam',
...
run_model(experiment, model, [x_train1], [y_train1, y_train2, y_train3], [x_test1], [y_test1, y_test2, y_test3], batch_size=batch_size, epochs=epochs)
