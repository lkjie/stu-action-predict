...
from keras.layers import LSTM, Input, Lambda, GRU
...
def build_model():
    ...
    for i in range(timeseries_count):
        ...
        emb = Embedding(input_dim=emb_timeseries_cates[i], output_dim=64, input_length=maxlen, mask_zero=False, trainable=True, name=emb_timeseries_names[i])(out)

        branch_outputs.append(emb)
    timeseries_x = keras.layers.concatenate(branch_outputs)

    lstm1 = GRU(512, dropout=0.2, recurrent_dropout=0.2, return_sequences=True)(timeseries_x)
    ...
    for i in range(feature_count):
        ...
        emb = Embedding(input_dim=emb_feat_cates[i], output_dim=64, mask_zero=False, trainable=True,
